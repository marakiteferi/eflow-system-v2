...
router.post('/approve', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (doc.submitter_id) {
            const submitterQuery = await client.query('SELECT name, email FROM users WHERE id = $1', [doc.submitter_id]);
        ...
        const executeSpawnNode = async (tNode) => {
            ...
            if (spawnIds) {
                ...
                for (const sId of ids) {
                    const wQuery = await client.query('SELECT name, flow_structure FROM workflows WHERE id = $1', [sId]);
                        ...
                        if (start && start.type === 'task') {
                            if (start.data?.assignmentStrategy === 'role_based') {
                                ...
                                if (start.data.routingType === 'SPECIFIC') {
                                    ...
                                    const submitterDeptQuery = await client.query('SELECT department_id FROM users WHERE id = $1', [doc.submitter_id]);
                                ...
                                const lbResult = await client.query(lbQuery, lbParams);
        ...
        if (doc.workflow_id && doc.current_node_id) {
            const wfQuery = await client.query('SELECT flow_structure FROM workflows WHERE id = $1', [doc.workflow_id]);
                        ...
                        if (branchNode && branchNode.type === 'task') {
                            ...
                            if (branchNode.data?.assignmentStrategy === 'role_based') {
                                ...
                                if (branchNode.data.routingType === 'SPECIFIC') {
                                    ...
                                    const sQuery = await client.query('SELECT department_id FROM users WHERE id = $1', [doc.submitter_id]);
...
router.post('/reject', authenticateToken, async (req, res) => {
    ...
    try {
        client = await pool.connect();
        await client.query('BEGIN');
        await client.query("UPDATE documents SET status = 'Rejected', updated_at = CURRENT_TIMESTAMP WHERE id = $1", [documentId]);

        const docQuery = await client.query('SELECT title, submitter_id, current_node_id FROM documents WHERE id = $1', [documentId]);
        const doc = docQuery.rows[0];

        await client.query("INSERT INTO approvals (document_id, approver_id, node_id, status, comments) VALUES ($1, $2, $3, 'Rejected', $4)", [documentId, req.user.id, doc.current_node_id || 'System', comments]);
        await client.query("INSERT INTO audit_logs (document_id, user_id, action) VALUES ($1, $2, 'Document Rejected - Revision Required')", [documentId, req.user.id]);
